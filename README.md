...
🔧 修改`_config.yml`和`_toc.yml`文件的内容，删除不需要的文件，添加新的Markdown文件（[MyST语法小抄](https://jupyterbook.org/en/stable/reference/cheatsheet.html)）；添加`.gitignore`文件，在其中添加`_build/`和其它不需要版本管理的文件；重新组织内容文档的文件目录结构。每次在本地仓库的`main`分支更新内容后，构建Jupyter Book以生成`.html`等网页文件：
